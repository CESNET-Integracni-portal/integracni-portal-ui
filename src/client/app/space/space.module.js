    ...
    function SpaceController($scope, $state, folder, labels, spaceService, fileuploadService, createFolderModal, uploadFilesModal, renameNodeModal, fileSaver) {
        ...
        function download() {
            ...
            if(vm.selectedNodes.length !== 1) {
                ...
                angular.forEach(vm.selectedNodes, function (node) {
                    if (node.type == 'folder') {
                        nodeGroup.folders.push(node.id);
                    }
                    else {
                        nodeGroup.files.push(node.uuid);
                    }
                });


                spaceService.downloadSelection(vm.space, nodeGroup)
                    .success(function(response){
                       fileSaver(response, "selection");
                    });
            }
            else {
                angular.forEach(vm.selectedNodes, function (node) {
                    if (node.type !== 'folder') {
                        ...
                        spaceService.downloadFolder(vm.space, node.id)
                            .success(function (response) {
                                fileSaver(response, node.name);
                            });
                    }
                });
            }
